...
def observation_adapter(env_obs):
    ...
    ego_ttc = np.array(ego_ttc, dtype=object)
    ego_lane_dist = np.array(ego_lane_dist, dtype=object)
    ...
    obs = {
        "distance_from_center": np.array([norm_dist_from_center], dtype=object),
        "angle_error": np.array([closest_wp.relative_heading(ego.heading)], dtype=object),
        "speed": np.array([ego.speed], dtype=object),
        "steering": np.array([ego.steering], dtype=object),
        "ego_lane_dist": np.array(ego_lane_dist, dtype=object),
        "ego_ttc": np.array(ego_ttc, dtype=object),
        "ego_will_crash": np.array([will_crash], dtype=object),
        "speed_of_closest": np.array([speed_of_closest], dtype=object),
        "proximity": proximity,
        "headings_of_cars": np.array(headings_of_cars, dtype=object),
    }
...
def ttc_by_path(ego, waypoint_paths, neighborhood_vehicle_states, ego_closest_wp):
    ...
    for v in neighborhood_vehicle_states:
        ...
        direction_vector = np.array(
            [math.cos(nearest_wp.heading), math.sin(nearest_wp.heading), ], dtype=object
        ...
        if lane_dist_by_path_index[path_idx] > lane_dist:
            ...
            if ego_closest_wp.lane_index == v.lane_index:
                speed_of_closest = (v.speed - ego.speed) / 33 #/ 120
